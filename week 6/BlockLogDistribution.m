...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% set observed factors
% all_var = unique([F.var]);
% non_query_var = setdiff(all_var, V);
% non_query_ass = [non_query_var',A(non_query_var)'];
% F_with_e = ObserveEvidence(F, non_query_ass);
% multiply_track = []; % tracks the factors already been multiplied
% fac = struct('var',[],'card',[],'val',[]);
% for i = 1:length(V)
%    cv = V(i);
%    rel_fac_name = G.var2factors{cv};
%    rel_factors = F(rel_fac_name);
%    for j = 1:length(rel_fac_name)
%        if(~ismember(rel_fac_name(j), multiply_track))
%             fac = FactorProduct(fac,(rel_factors(j)));
%             multiply_track = [multiply_track, rel_fac_name(j)];
%        end
%    end
% end
% f_marg = FactorMarginalization(fac,V);
% denominator = GetValueOfAssignment(f_marg, A(f_marg.var), f_marg.var);
% for s = 1:d
%    assign = A;
%    assign(V) = s;
%    assign = assign(fac.var);
%    numerator = GetValueOfAssignment(fac, assign, fac.var);
%    LogBS(s) = numerator/denominator;
% end
% LogBS = log(LogBS);
assign = repmat(A, d, 1);
assign(:, V) = repmat((1:d)', 1, length(V));

fIndices = unique([G.var2factors{V}]);
for f=F(fIndices)
    v = GetValueOfAssignment(f, assign(:, f.var));
    LogBS = LogBS + log(v);
end
