...
q_list = G.q_list;
q_keep_indx = find(A(q_list(:, 1)) == A(q_list(:, 2))); % find two connected nodes has the same assign
q_list = q_list(q_keep_indx, :); % only keep those has the same assign
...
[var2comp, cc_sizes] = scomponents(SelEdgeMat); % var2comp refers to the index of the complex of each variable
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
LogPropR = LogR(old_value(1)) - LogR(new_value);
LogQ = LogPropR + log_QY_ratio;
logAcc = LogProbOfJointAssignment(F, A_prop) - LogProbOfJointAssignment(F, A);
p_acceptance  = min(exp(logAcc+LogQ), 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
