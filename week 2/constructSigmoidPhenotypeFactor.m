...
permu = IndexToAssignment(1:prod(phenotypeFactor.card), phenotypeFactor.card);
for i = 1:2:length(permu)
    z = 0;
    for j = 2:length(phenotypeFactor.card)
        gene_index = 1+mod(j,2);
        z = z + alleleWeights{gene_index}(permu(i,j));
    end
    phenotypeFactor.val(i) = exp(z)/(1+exp(z));
