...
for i = 1:numPeople*2 % P_copy1
    k = mod(i,numPeople);
    if k == 0
        k = numPeople;
    end
    r = ceil(i/numPeople);
    if any(pedigree.parents(k,:)) % has parents
        factorList(i).var = [i, pedigree.parents(k,r), pedigree.parents(k,r)+numPeople];
        factorList(i).card = ones(1,length(factorList(i).var)) * numAlleles;
        factorList(i).val = zeros(1,prod(factorList(i).card));
        geneList = IndexToAssignment(1:length(factorList(i).val), factorList(i).card);
        for m = 1 : length(geneList)
           Cgene = geneList(m,1);
           Pgene1 = geneList(m,2);
           Pgene2 = geneList(m,3);
           factorList(i).val(m) = 0.5 * sum([Pgene1, Pgene2] == Cgene);
        end
...
for i = numPeople*2+1 : numPeople*3
    k = mod(i,numPeople);
    if k == 0
        k = numPeople;
    end
    factorList(i) = phenotypeGivenCopiesFactor(alphaList, numAlleles, k, k + numPeople, i);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
