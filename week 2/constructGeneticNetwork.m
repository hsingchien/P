...
for i = 1 : numPeople
   if any(pedigree.parents(i,:)) % has parents
       factorList(i) = genotypeGivenParentsGenotypesFactor(numAllele, i, pedigree.parents(i,1), pedigree.parents(i,2));
   else % do not have parents
...
for i = numPeople+1 : 2*numPeople
    genoVar = i - numPeople;
    phenoVar = i;
    factorList(i) = phenotypeGivenGenotypeFactor(alphaList, genoVar, phenoVar);
end
