...
eventlist = IndexToAssignment(1:length(genotypeFactor.val),genotypeFactor.card);
for i = 1:length(genotypeFactor.val)
    genoC = genotypesToAlleles(eventlist(i,1),:);
    genoP1 = genotypesToAlleles(eventlist(i,2),:);
    genoP2 = genotypesToAlleles(eventlist(i,3),:);
    possible_offspring = combvec(genoP1,genoP2)';
    genotypeFactor.val(i) = 0;
    for j = 1:length(possible_offspring)
        if (possible_offspring(j,1) == genoC(1) & possible_offspring(j,2) == genoC(2)) ...
                | (possible_offspring(j,1) == genoC(2) & possible_offspring(j,2) == genoC(1))
