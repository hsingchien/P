...
%     visited = [];
%     fac_scale = C.nodes{i};
%     target = struct('var',[],'card',[],'val',[]);
%     for j = 1:length(temp)
%         current = temp(j);
%         if(length(intersect(current.var, fac_scale)) == length(current.var))
%             target = FactorProduct(current, target);
%             visited = [visited, j];
%         end
%     end
%     temp(visited) = [];
%     P.cliqueList(i) = SortVar(target);
% end
